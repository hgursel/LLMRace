...
def test_provider_mode_mapping() -> None:
    ...
    assert provider_mode(ConnectionType.OPENAI) == 'openai_compat'
    assert provider_mode(ConnectionType.OPENROUTER) == 'openai_compat'
    assert provider_mode(ConnectionType.ANTHROPIC) == 'anthropic'
